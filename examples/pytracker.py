import cv2
import numpy as np
from lib.utils import get_img_list,get_init_gt,APCE,PSR
import time
...
from cftracker.autotrack import AutoTrack
...
#            writer = cv2.VideoWriter(video_path, cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), 30, (init_frame.shape[1], init_frame.shape[0]))
        ...
        for idx in range(len(self.frame_list)):
            if idx != 0:
                ...
                timestart=time.time()
                bbox=self.tracker.update(current_frame,idx+1,vis=verbose)
                delta_t=time.time()-timestart
                ti+=delta_t
                x1,y1,w,h=bbox
                if verbose is True:
...
#                    current_frame[ymin:ymax, xmin:xmax] = score_map
                    ...
                    cv2.putText(show_frame, 'FPS:' + str(1/delta_t), (0, 250), cv2.FONT_HERSHEY_COMPLEX, 1,
                                (0, 0, 255), 5)

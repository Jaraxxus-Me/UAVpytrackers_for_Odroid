...
def ifft2_sym(x):
    if np.shape(x)[0]%2 != 0:
        return np.real(np.fft.ifft(np.fft.ifft(x, axis=1), axis=0).astype(np.complex64))
    else:     
        lenx = int(np.shape(x)[0]/2+1)
        leny = int(np.shape(x)[1]/2+1)
        return np.fft.irfft(np.fft.irfft(x[:,0:leny],axis = 1)[0:lenx,:],axis = 0).astype(np.float32)

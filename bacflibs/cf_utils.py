...
def resize_dft2(input_dft, desired_sz):
    con = 0
    if len(np.shape(input_dft)) == 2:
        input_dft = input_dft[:,:,np.newaxis]
        con = 1
    ...
    if desired_sz[0] != w or desired_sz[1] != h:
        ...
        if con == 1:
            resize_dfft = np.squeeze(resize_dfft)
        return resized_dfft
    else:
        if con == 1:
            input_dft = np.squeeze(input_dft)
        return input_dft
...
def resp_newton(response, responsef, iterations, ky, kx, use_sz):
    if len(response.shape)<3:
        ...
        responsef=responsef[:,:,np.newaxis]
